...
| **Orderbook** | Per-merchant Orderbook module: enable/disable toggle, config (min/max spread, order size limits, exposure limit, quote rate limit, auto-cancel timeout, allowed sides, taker fee bps, maker rebate bps, kill-switch, KYC requirement), 8-stat dashboard (markets, quotes, depth, exposure, fills 24h, P&L 24h, avg spread, quoting uptime), Platform Risk Controls (Throttle / Kill All Quotes / Suspend) |
| **Betting** | Per-merchant BC parimutuel module: enable/disable toggle, config (min/max stake, max payout, house fee bps, creator fee share bps, settlement window, cooldown, max open markets/user, allowed categories, auto-void no liquidity, KYC requirement), 8-stat dashboard (markets, bets 24h, handle 24h, GGR 24h, avg stake, settlement rate, pending settlements, active bettors), Platform Risk Controls (Pause New Bets / Force Settlement / Suspend Module) |
| **Fast Bet** | Per-merchant Fast Bet module (independent from Betting): rapid binary up/down tick markets with fixed payout multiplier. Config (min/max stake, tick duration, payout multiplier, house edge bps, cooldown between bets, max bets/min, max daily loss/user, underlying asset, KYC, daily loss cap, streak bonus), 8-stat dashboard (active ticks, bets 24h, handle 24h, GGR 24h, avg bet, user win rate, active users, avg session), Platform Risk Controls (Pause Ticks / Void Current Tick / Suspend Module) |
...
| MD-08 | Per-merchant Orderbook module enable/disable + config + stats + risk override | P0 |
| MD-09 | Per-merchant Betting (BC parimutuel) module enable/disable + config + stats + risk override | P0 |
| MD-10 | Per-merchant Fast Bet module (independent of Betting) enable/disable + config + stats + risk override | P0 |
...
| **1.7** | 2026-04-29 | **Branding & Auth simplification**: ForeGate logo image asset replaced with PrediEx text wordmark across sidebar, signup, login, MFA pages, home, creator landing. Post-signup/login MFA setup page removed entirely (route `/mfa-setup` deleted, `mfa-setup.tsx` deleted, user-layout no longer force-redirects); users go directly to `/`. MFA management is still reachable via `mfa-manager.tsx` (Login Preferences card removed; first "Configure Your MFA Methods" heading removed, retaining only the second). **Orderbook surfaces (3 coordinated views)**: (1) **User** — Open Orders card on user-facing market detail; visible inside `OrderbookTradingPanel` on desktop and as a separate `lg:hidden` body card on mobile (action/side badges, fill progress bar, remaining/filled counts, cancel button). My Positions removed from trading panel; restored as a body section on orderbook market detail (works on both mobile and desktop). (2) **Merchant** — `/oms/orderbook` (`merchant-orderbook.tsx`): self-management page for merchants with Active Quotes/Depth/Exposure/Fills/P&L stats, My Markets table, My Quotes table with cancel + filters, Post Quote modal, Kill All button. (3) **Platform** — moved from a standalone module into a per-merchant **Orderbook** tab on `merchant-detail.tsx` (only merchants who enable the module see config & stats). **Per-merchant module tabs added to merchant-detail.tsx**: Orderbook, Betting (BC parimutuel), and Fast Bet (independent from Betting — rapid binary tick markets with payout multiplier). Each tab has: enable/disable toggle (default mocked from merchant id seed), 8-stat dashboard, full configuration form, kill-switch / KYC toggles, Save/Reset, and Platform Risk Controls panel. New functional requirements MD-08, MD-09, MD-10 added to §6.3. | (1) **WebSocket real-time event stream** for multi-admin collaboration — simulated WS feed with 15 event templates across 9 event types (admin_join/leave, transition, dry_run, dry_run_commit, system, alert, batch_progress, dispute_filed), admin presence indicators with avatar stacking in header + queue table viewers column, auto-scrolling feed with resume control, 4 mock connected admins with role/viewing-job context. (2) **Settlement dry-run mode** — payout preview modal before committing transitions for eligible states (resolved→settling, disputed→resolved), calculates winning/losing/voided bet distribution, full payout breakdown table with platform fee (5%), wallet impact analysis (hot wallet before/after, cold reserve, top-up required flag) with utilization bar, dynamic risk flags (high volume, disputed, wallet threshold, retry idempotency, abnormal win rate), estimated settlement duration, commit-with-spinner UX, all actions logged to audit trail and emitted to WS event stream. |

---

*End of PRD - PredictEx PaaS OMS v1.7 (Updated: April 29, 2026)*
